...
export class TransactionService {
    ...
    public async makePayment(user: User, planId: string) {
        const plan = await this.prismaService.sponsorshipPlan.findUniqueOrThrow({
            where: {
                id: planId
            },
            include: {
        ...
        if (!plan.channel) {
            throw new NotFoundException("Channel not found for this plan")
        }
    
        if (user.id === plan.channel.id) {
            throw new ConflictException(
                'You cannot arrange sponsorship on yourself'
            )
        }
        ...
        if (existingSubscription) {
            throw new ConflictException(
                'You have already issued sponsorship for this channel'
            )
        }
