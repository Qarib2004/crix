...
export class FollowService {
	...
	public async follow(user: User, channelId: string) {
		...
		if (existingFollow) {
			throw new ConflictException('You are already signed for this channel')
		}
        ...
        if (follow.following.notificationSettings?.siteNotifications) {
            await this.notificationService.craeteNewFollowing(
              follow.following.id,
              follow.follower
            )
          }
	...
	public async unfollow(user: User, channelId: string) {
		...
		if (channel.id === user.id) {
			throw new ConflictException('You can not unsubscribe to yourself')
		}
		...
		if (!existingFollow) {
			throw new ConflictException('You are not signed for this channel')
		}
