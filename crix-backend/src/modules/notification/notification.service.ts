import {NotificationType,
	...
	TokenType, type User } from '@/generated';
...
export class NotificationService {
	...
	public async createStreamStart(userId: string, channel: User) {
		const notification = await this.prismaService.notification.create({
			data: {
				...
				<p>Join Stream on the channel <a href='/${channel.username}' className='font-semibold'>${channel.displayName}</a>.</p>`,
    ...
    public async craeteNewFollowing(userId:string,follower:User){
        const notification = await this.prismaService.notification.create({
            data:{
	...
	public async createNewSponsorship(
		...
		const notification = await this.prismaService.notification.create({
			data: {
				...
				<p>User <a href='/${sponsor.username}' className='font-semibold'>${sponsor.displayName}</a> He became your sponsor by choosing a plan <strong>${plan.title}</strong>.</p>`,
	...
	public async createEnableTwoFactor(userId: string) {
		const notification = await this.prismaService.notification.create({
			data: {
				message: `<b className='font-medium'>Ensure your security!</b>
				<p>Turn on two -factor authentication in the settings of your account to increase the level of protection.</p>`,
	...
	public async createVerifyChannel(userId: string) {
		const notification = await this.prismaService.notification.create({
			data: {
			  ...
			  <p>Your channel is verified, and now there will be a checkbox next to your channel.</p>`,
